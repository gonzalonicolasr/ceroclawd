...
import { DEFAULT_CEROCLAW_MODEL, MAINLINE_CODER_MODEL } from '../config/models.js';
...
export const AUTH_ENV_MAPPINGS = {
  openai: {
    ...
    model: ['OPENAI_MODEL', 'CEROCLAW_MODEL'],
  },
  anthropic: {
  ...
  ollama: {
    ...
    model: ['OLLAMA_MODEL', 'CEROCLAW_MODEL'],
  },
  glm: {
    apiKey: ['GLM_API_KEY'],
...
/**
 * Derive allowed models from CEROCLAW_OAUTH_MODELS for authorization.
 * This ensures single source of truth (SSOT).
 */
export const CEROCLAW_OAUTH_ALLOWED_MODELS = CEROCLAW_OAUTH_MODELS.map(
  (model) => model.id,
) as readonly string[];
