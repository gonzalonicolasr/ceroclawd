...
export function validateAuthMethod(
  ...
  if (authMethod === AuthType.CEROCLAW_OAUTH) {
    // Ceroclawd OAuth doesn't require any environment variables for basic setup
  ...
  if (authMethod === AuthType.USE_CHATGPT_OAUTH) {
    const tokensFile = path.join(os.homedir(), '.ceroclawd', 'chatgpt-tokens.json');
    if (!fs.existsSync(tokensFile)) {
      return t('ChatGPT not authenticated. Run: ceroclawd auth chatgpt');
    }
    return null;
  }
